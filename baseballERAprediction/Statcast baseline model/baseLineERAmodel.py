...
history = model.fit(np.array(train_features),np.array(train_labels),batch_size=20 , epochs=1000 , verbose=1,validation_split = 0.2) 
...
if 0 == 0:
    ...
    input_fastball_avg_spin = list(input['fastball_avg_spin'])

    
    input_features =  list(zip(input_k_percent,input_fastball_avg_speed,input_batting_avg,input_whiff_percent,input_launch_angle_avg,input_fastball_avg_spin))# ,data['k_percent'],data['batting_avg'],data['exit_velocity_avg'],data['launch_angle_avg'],data['barrel_batted_rate'],data['hard_hit_percent'],data['avg_best_speed,avg_hyper_speed'],data['whiff_percent'],data['f_strike_percent'],data['ff_avg_speed'],data['fastball_avg_speed'],data['fastball_avg_spin'],data['breaking_avg_spin'],data['offspeed_avg_spin']# features = list(zip(buying,maint,doors, persons,lug_boot,safety))# label = list(Class)
...
plt.plot(input_predictions,input_label)
plt.ylim([0, 10])
plt.xlabel('predictedERA')
plt.ylabel('actualEra')
plt.legend()
plt.grid(True)
plt.show()
